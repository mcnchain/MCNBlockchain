/***************************************************************
 * split-all-gas.js — делим ГАЗ КАЖДОЙ ТРАНЗАКЦИИ в сети
 * v3.4 — считаем газ по всем tx, НО игнорим исходящие от COINBASE
 *        (и других указанных адресов), чтобы не раздавать свой же газ.
 *        Лок от реэнтри + один активный триггер.
 ***************************************************************/
...
var VALIDATOR_WEIGHTS = {}; // пусто = поровну

// Адреса-исключения (газ этих отправителей НЕ учитываем в пуле)
var COINBASE = eth.coinbase; if (!COINBASE) throw "No etherbase set";
var EXCLUDE_SENDERS = (function(){
  var s = {};
  s[COINBASE.toLowerCase()] = true; // игнорим материнский
  // при желании добавь ещё:
  // s["0x...".toLowerCase()] = true;
  return s;
})();

// Проверь адрес сжигания!
var BURN_ADDR = "0x000000000000000000000000000000000000dEaD";
// var BURN_ADDR = "0x57d5937C6942B5f59c44c7400834A4946C397734"; // test burn
...
// 1) новый блок — единственный активный триггер
...
/*
// 2) Если нужно — включи резервный таймер (оставь ЛОК ВКЛЮЧЁННЫМ)
if (typeof _lastBlockNum === "undefined") { _lastBlockNum = eth.blockNumber; }
if (typeof _splitTimerStarted === "undefined"){
  _splitTimerId = setInterval(function(){
    try {
      var n = eth.blockNumber;
      if (n !== _lastBlockNum){
        _lastBlockNum = n;
        processBlocksAndDistribute();
      }
    } catch(e){ console.log("tick error:", e); }
  }, 3000);
  _splitTimerStarted = true;
  console.log("split-all-gas: timer trigger enabled");
}
*/
